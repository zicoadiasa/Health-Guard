...
The system derives Body Mass Index (BMI) and Basal Metabolic Rate (BMR) from the user's stored height, weight, gender, birth date, and activity level. These values are calculated by database functions to ensure consistent health recommendations across application services.
...
Notifications may be generated automatically from reminder schedules or other application events through PostgreSQL database functions. This automation ensures consistent notification creation across the system.
...
Weekly reports are generated by aggregating user activity, food logs, blood sugar logs, weight logs, and goals through a database reporting function.
